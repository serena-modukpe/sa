...
export default function ProfilIndex() {
    ...
    const fetchProfils = async () => {
        try {
            const response = await Profilsservices.index(token);
    ...
    const goToEdit = (param) => {
        navigate(myroutes.profils_edit, { state: { id: param } })
    }
    ...
    const onDelete = (param) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
              Profilsservices.delete(param, token).then((response) => {
                ...
                <div className="app-content pt-3 p-md-3 p-lg-4 app-card app-card-stat">
                    <div className="container-xl app-card-body">
                        ...
                        <div className="row g-3 mb-4 align-items-center justify-content-between">
                            <div className="col-auto">
                                <h3 className="app-page-title mb-0">Liste des Profils <Link className="btn btn-success btn-sm" to={myroutes.profils_create} title="Ajouter"><i className="bi bi-plus"></i>Ajouter</Link> </h3>
                            </div>
                        </div>{/*//row*/}
                                    ...
                                    <table className="table table-striped table-font table-hover app-table-hover mb-0 data-table">
                                        <thead className="table-info mt-2">
                                            <tr>
                                                ...
                                                <th className="text-center">Habilitations</th>
                                        ...
                                        <tbody className="table-group-divider" style={{ textAlign: "left" }}>
                                                    {profils.length > 0 && profils.map((item, index) => (
                                                    <tr key={index}>
                                                        <td className="text-center">{index + 1}</td>
                                                        <td className="text-center">{item && item.get_user.nom ? item.get_user.nom : 'N/A'}</td>
                                                        <td className="text-center">{item && item.get_role.libelle ? item.get_role.libelle : 'N/A'}</td>
                                                        
                                                        <td className="text-center">{item && item.get_habilitation.libelle ? item.get_habilitation.libelle : 'N/A'}</td>
